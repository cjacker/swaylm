...
impl<'a> LoginManager<'a> {
    ...
    fn draw_bg(&mut self, box_color: &Color) -> Result<(), Error> {
        ...
        let (username_color, password_color, _) = match self.mode {
...
*/
    ...
    fn greeter_loop(&mut self) {
        ...
        let startsway: String = String::from("startsway");
        let target_cmd: Vec<String> = vec![startsway];
                    ...
                    Mode::EditingPassword => {
                        if password.is_empty() {
                            ...
                            let res = self.greetd.login(
                                username,
                                password,
                                target_cmd.clone(), //self.targets[self.target_index].exec.clone(),
                            );
...
fn main() {
    ...
    let mut lm = LoginManager::new(&mut framebuffer, (w, h), (1024, 168), greetd);
